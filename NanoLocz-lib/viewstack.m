...
addlistener(hSlider, 'Value', 'PostSet', @(src, event) updateSlice(round(hSlider.Value),varargin{:}));
    ...
    function updateSlice(idx , varargin)
        ...
        % --- Draw labels if checkbox checked ---
        if hDrawLabels.Value
            % Clear existing lines and text to prevent overplot
            delete(findall(hAx, 'Type', 'line'));
            delete(findall(hAx, 'Type', 'text'));
if ~isempty(varargin)
    Info = varargin{1};
    
    drawLabels_v2(hAx, stack(:, :, currentSlice), currentSlice, Info, true, true);
end
            % Draw labels
           
        else
            % Clear labels if drawlabels just turned off
            delete(findall(hAx, 'Type', 'line'));
            delete(findall(hAx, 'Type', 'text'));
        end
    end
